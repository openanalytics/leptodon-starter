...
1. `npm install`
2. Run tailwind's css generator `npx tailwindcss -i input.css -o style/output.css --watch`. Tailwind is configured via `tailwind.config.js` and manual css can go at the end off `input.css`.
3. Setup commit hooks `pre-commit install`, may need to install pre-commit first. Pre-commit is configured via [.pre-commit-config.yaml](./.pre-commit-config.yaml).
4. `direnv allow` to enable the direnv in case you use nixos with flakes. The direnv is configured via `.envrc`, `flake.nix` and `flake.lock`.
5. `cd end2end; direnv allow` to enable the playwright devenv, you may need to install `devenv`.
    1. `npm install`. You also need to install playwright here.
6. `playwright test` to run the tests.
7. `cargo install --locked cargo-make` to enable Makefile.toml support, useful commands include `cargo make mimic-ci` and `cargo make build-starter`.
